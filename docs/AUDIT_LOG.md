...
identificador técnico do alvo, e data/hora. A tabela não possui coluna de
metadata; assim, não persiste nomes, e-mails, conteúdo enviado, valores de
negócio, senhas, tokens, cookies, chaves, cabeçalhos ou outros dados sensíveis.
...
(365 dias por padrão) antes de operar em produção. Esta versão ainda não
executa prune automático: a variável é uma política declarada e o produto deve
implementar o descarte antes de tratá-la como retenção efetivamente aplicada.
Backups e exportações devem seguir uma política própria de acesso e retenção.
